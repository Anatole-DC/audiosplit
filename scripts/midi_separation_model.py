...
class MIDIDataProcessor:
    ...
    def _piano_roll_to_notes(self, piano_roll, threshold=0.5):
        ...
        for pitch in range(piano_roll.shape[1]):
            ...
            note_changes = np.diff(
                np.concatenate(([False], pitch_roll, [False])).astype(int)
            )
        ...
        for midi_file in tqdm(midi_files):
            ...
            if augment:
                for shift in [-2, -1, 1, 2]:  # Shift by semitones
                    shifted_input, shifted_target = self._pitch_shift(
                        mixed_roll, target_stack, shift
                    )
    ...
    def __getitem__(self, idx):
        ...
        if input_roll.shape[0] > self.sequence_length:
            ...
            target_seq = np.pad(
                target_roll,
                (
                    (0, max(0, self.sequence_length - target_roll.shape[0])),
                    (0, 0),
                    (0, 0),
                ),
                mode="constant",
...
class InstrumentSeparationModel(nn.Module):
    ...
    def __init__(
        self, input_size=128, hidden_size=256, num_instruments=8, num_layers=3
    ):
        """
...
class MIDISeparationTrainer:
    ...
    def save_model(self, filepath):
        ...
        torch.save(
            {"model_state_dict": self.model.state_dict(), "history": self.history},
            filepath,
        )
...
def separate_midi_file(model, processor, input_file, output_dir, device="cpu"):
    ...
    output_files = processor.piano_roll_to_midi(
        separated_rolls, output_dir, Path(input_file).stem
    )
