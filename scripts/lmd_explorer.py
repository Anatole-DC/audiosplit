...
class LakhMIDIExplorer:
    ...
    def download_dataset(
        self, components=["md5_to_paths", "match_scores"], force_download=False
    ):
        """
    ...
    def scan_midi_files(self, subset="all", max_files=None):
        ...
        if subset == "all" or subset == "full":
            midi_paths = (
                list(self.lmd_full_path.glob("**/*.mid*"))
                if self.lmd_full_path.exists()
                else []
            )
        elif subset == "matched":
            midi_paths = (
                list(self.lmd_matched_path.glob("**/*.mid*"))
                if self.lmd_matched_path.exists()
                else []
            )
        elif subset == "aligned":
            midi_paths = (
                list(self.lmd_aligned_path.glob("**/*.mid*"))
                if self.lmd_aligned_path.exists()
                else []
            )
        else:
            midi_paths = []
    ...
    def analyze_midi_file(self, midi_path):
        ...
        try:
            ...
            analysis = {
                ...
                "num_drum_tracks": len(
                    [inst for inst in midi.instruments if inst.is_drum]
                ),
                ...
                "programs": [
                    inst.program for inst in midi.instruments if not inst.is_drum
                ],
                "instruments": [
            ...
            return {"filename": midi_path.name, "error": str(e), "valid": False}
    ...
    def generate_statistics(self):
        ...
        valid_files = [
            f for f in self.analysis_results["files"] if f.get("valid", False)
        ]
        ...
        stats = {
            ...
            "tempo": {
                "mean_initial": df["tempo_stats"]
                .apply(lambda x: x["initial_tempo"])
                .mean(),
    ...
    def create_visualizations(self, save_path=None):
        ...
        if not self.analysis_results or not self.midi_stats:
            print(
                "❌ No analysis results available. Run batch_analyze() and generate_statistics() first."
            )
            return
        ...
        axes[0, 1].hist(
            df["num_instruments"],
            bins=range(0, df["num_instruments"].max() + 2),
            alpha=0.7,
            color="lightgreen",
        )
        ...
        axes[2, 1].bar(
            x - width / 2, with_counts, width, label="With", color="lightblue"
        )
        axes[2, 1].bar(
            x + width / 2, without_counts, width, label="Without", color="lightgray"
        )
        ...
        axes[2, 2].scatter(
            df["duration"], df["total_notes"], alpha=0.6, color="darkblue"
        )
    ...
    def print_summary_report(self):
        ...
        print(
            f"   Success rate: {summary['valid_files']/summary['total_files']*100:.1f}%"
        )
        ...
        print(
            f"   Mean duration: {duration_stats['mean']:.1f} seconds ({duration_stats['mean']/60:.1f} minutes)"
        )
        print(f"   Median duration: {duration_stats['median']:.1f} seconds")
        print(f"   Shortest file: {duration_stats['min']:.1f} seconds")
        print(
            f"   Longest file: {duration_stats['max']:.1f} seconds ({duration_stats['max']/60:.1f} minutes)"
        )
        ...
        # Most common instruments
        print(f"\n🎸 TOP 10 INSTRUMENTS")
        for i, (instrument, count) in enumerate(
            self.midi_stats["common_instruments"][:10], 1
        ):
    ...
    def export_results(self, filename="lmd_analysis_results.json"):
        ...
        export_data = {
            ...
            "metadata": {
                ...
                "total_md5_mappings": (
                    len(self.md5_to_paths) if hasattr(self, "md5_to_paths") else 0
                ),
                "total_match_scores": (
                    len(self.match_scores) if hasattr(self, "match_scores") else 0
                ),
            },
        }
